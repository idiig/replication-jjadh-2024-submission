...
### 翻訳一般性仮説 (Translation universal hypothesis)
...
$$
\begin{align}
\text{U}_{T}(S,T) &= 1 - \text{agreement}_{T}(S,T)\\
...
$|T|$ はターゲットテキストの長さを指し、$s_i$, $t_i$ はそれぞれソーステキスト、ターゲットテキストの $i$ 番目と $j$ 番目の要素を意味する。追加率は $1 - \text{agreement}_{T}(S,T)$ で計算する。

一方で、不一致率は、原文にある要素のうち、現代語訳において対応が存在しない割合として、同様な計算ができる。

$$
\begin{align}
\text{U}_{S}(S,T) &= 1 - \text{agreement}_{S}(S,T)\\
\text{agreement}_{S}(S,T) &= \frac{1}{|S|}\sum_{s_i}^{|s|} \max_{t_j \in T} \text{score}(t_i, s_j)
\end{align}
$$
...
(a) 追加率は、翻訳を問わず 50% 前後、追加要素を加えていることがわかった。この点は、観測データ通りであった。
(b) 翻訳観グループで比較した結果、2 グループの差の事後分布の 95% の CrI はすべて 0 をカバーしており、それぞれの 2 群の追加率は異なることが認められなかった。
傾向としては、`Reader-focused` 群の追加率に比べ `Other` 群の追加率が 5% 程度下回り (CrI $[-0.13, 04]$, $P(\Delta > 0) = 88\%$)、`Poet-focused` 群が 5% 程度下回る (CrI $[-0.13, 04]$, $P(\Delta > 0) = 87.6\%$) ことが観測されているが、傾向差の基準を満していない。
...
pred_addition_rate_diff  |>
  ...
  annotate(
    ...
    hjust = 1,
    size = 3,
    label = pred_diff_CrI_annotation,
    parse = TRUE
  ) + 
  annotate(
    geom="text",
    ...
    hjust = 0,
    size = 3,
    label = pred_diff_prob_g_0_annotation,
    parse = TRUE
  ) + 
  geom_vline(
    xintercept = 0, 
